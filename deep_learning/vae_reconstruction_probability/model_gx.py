...
# paper_address: http://dm.snu.ac.kr/static/docs/TR/SNUDM-TR-2015-03.pdf
# github: https://github.com/Michedev/VAE_anomaly_detection
...
class VAEAnomalyDetection(nn.Module, ABC):
    ...
    def reparameterize(self, z_mu, z_log_var):
        batch_size=len(z_mu)
        variance = torch.exp(z_log_var/2)
        distribution = Normal(z_mu, variance) # batch_size, latent_size
        ####### method 1 ########
        # eps = torch.randn(z_mu.size(0)*self.L, z_mu.size(2)).to(z_mu.get_device())
        # new_z = eps*torch.exp(z_log_var/2.) + z_mu
        # new_z = new_z.view(-1, self.latent_size)

        ####### method 2 ########
        new_z = distribution.rsample([self.L]) # [L, batch_size, latent_size]
        print('new_z.shape: ', new_z.shape)
        new_z = new_z.view(self.L * batch_size, self.latent_size)
        return new_z, distribution
    ...
    def predict(self, x) -> dict:
        ...
        """
        batch_size = len(x)
        latent_mu, latent_log_var = self.encoder(x).chunk(2, dim=1) # both with size [batch_size, latent_size], split like this: a1 | a2 
        print("lantent_mu shape:", latent_mu.size())
        # latent_sigma = softplus(latent_sigma) # Like ReLU, but has continuing gradients at x=0, doesn't have any param to learn
        # dist = Normal(latent_mu, latent_sigma) # batch_size, latent_size
        # z = dist.rsample([self.L])  # shape: [L, batch_size, latent_size]
        # z = z.view(self.L * batch_size, self.latent_size)
        z , dist = self.reparameterize(latent_mu, latent_log_var)
        
        recon_mu, recon_log_var = self.decoder(z).chunk(2, dim=1)
        recon_sigma = torch.exp(recon_log_var/2.) # convert it to >=0, because std should be >0.
        recon_mu = recon_mu.view(self.L, *x.shape) # L, Batch, Latent_size
        recon_sigma = recon_sigma.view(self.L, *x.shape) # L, Batch , Latent_size
        return dict(latent_dist=dist, latent_mu=latent_mu,
                    latent_log_var=latent_log_var, recon_mu=recon_mu,
                    recon_sigma=recon_sigma, z=z)


    def forward(self, x):
        pred_result_collection = self.predict(x)
        # unsqueeze to broadcast input across sample dimension (L) (B,D -> 1,B,D -> L, B, D)
        x = x.unsqueeze(0)
        # average over sample dimension -> B, 1
        log_lik = Normal(pred_result_collection['recon_mu'], 
                        pred_result_collection['recon_sigma']).log_prob(x).mean(dim=0)
        print("forward log_lik shape (1):", log_lik.size())
        # B,1 -> 1,1 -> 1,
        log_lik = log_lik.mean(dim=0).sum()
        print("forward log_lik shape (1):", log_lik.size())
        kl = kl_divergence(pred_result_collection['latent_dist'], self.prior).mean(dim=0).sum()
        loss = kl - log_lik
        return dict(loss=loss, kl=kl, recon_loss=log_lik, **pred_result_collection)
    ...
    def reconstructed_probability(self, x):
        ...
        print("reconstructed_prob, input x shape:", x.size())
        x = x.unsqueeze(0) # for broadcast
        print("after unsqueezed, x shape:" x.size())
