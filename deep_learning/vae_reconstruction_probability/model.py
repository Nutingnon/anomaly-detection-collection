...
class VAEAnomalyDetection(nn.Module, ABC):
    ...
    def predict(self, x) -> dict:
        ...
        """
        batch_size = len(x)
        latent_mu, latent_sigma = self.encoder(x).chunk(2, dim=1) # both with size [batch_size, latent_size], split like this: a1 | a2 
        ...
        recon_sigma = softplus(recon_sigma) # convert it to >=0, because std should be >0.
